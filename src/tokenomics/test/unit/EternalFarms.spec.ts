...
import { IAlgebraVirtualPool } from '@cryptoalgebra/v1.9-ve-core/typechain'
    ...
    describe('fails when', () => {
      ...
      it('extremely huge liquidity', async () => {
        ...
        const tokenId2 = await mintPosition(context.nft.connect(lpUser0), {
          ...
          recipient: lpUser0.address,
          amount0Desired: BigNumber.from(2).pow(128).sub(1).div(BigNumber.from(887272 * 2)).div(2),
          amount1Desired: BigNumber.from(2).pow(128).sub(1).div(BigNumber.from(887272 * 2)).div(2),
          amount0Min: 0,
          amount1Min: 0,
          deadline: (await blockTimestamp()) + 1000,
        })
